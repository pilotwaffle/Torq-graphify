...
def main() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        ...
        print("  fitness                 graph health metrics + PASS/LOW/FAIL verdict")
        print("    --profile <name>        score the named graphify.toml profile's graph")
        print("    --graph <path>          path to graph.json (default: resolved output dir)")
        print("    --json                  machine-readable report")
        print("    --strict                LOW verdict also exits 1 (CI gating)")
        print("  profiles                list graph profiles defined in graphify.toml")
